classdef Dynamics < handle
 ...
 properties(Constant)
    %--------------------------------Body--------------------------------------------------------------------------
    m0=0.7; L=0.23; Ix=0.001915; Iy= 0.001630;
    Iz=0.003147;
    g=9.81;
    %--------------------------------Arm1--------------------------------------------------------------------------
    m1=0.1; L1=0.1; l1=Dynamics.L1/2; I1=diag([2.5*10^(-5),1*10^(-4),2.5*10^(-5)]);tau1_s=100000;
    %--------------------------------Arm2---------------------------------------------------------------------------
    m2=0.1; L2=0.1; l2=Dynamics.L2/2; I2=diag([2.5*10^(-5),1*10^(-4),2.5*10^(-5)]);tau2_s=100000;
 end
...
%-----------------------------------------------------------------------------------------------------
        ...
        %% h Matrix
        Cb=zeros(8,1);
        Cb(4)=2*Sth*Cth*(C.Ix-Sps^2*C.Iy-Cps^2*C.Iz)*ph_dot*th_dot  +(-Cth*C.Ix+Cth*(Cps^2-Sps^2)*(C.Iy-C.Iz))*x(11)*x(12)  +2*Cth^2*Sps*Cps*(C.Iy-C.Iz)*x(12)*x(10)  -Sps*Cps*Sth*(C.Iy-C.Iz)*x(11)^2;
        Cb(5)=2*Cps*Sps*(C.Iz-C.Iy)*x(11)*x(12)  +Cth*(Cps^2-Sps^2)*(C.Iy-C.Iz)*x(12)*x(10)  -Sth*Sps*Cps*(C.Iy-C.Iz)*x(10)*x(11) -Sth*Cth*(C.Ix-Sps^2*C.Iy-Cps^2*C.Iz)*ph_dot^2  +Sth*Sps*Cps*(C.Iy-C.Iz)*ph_dot*th_dot  +Cth*C.Ix*ps_dot*ph_dot;
        Cb(6)=-Cth*C.Ix*x(10)*x(11)-Cth^2*Sps*Cps*(C.Iy-C.Iz)*ph_dot^2-Sps*Cps*(C.Iz-C.Iy)*th_dot^2-(Sps^2-Cps^2)*Cth*(C.Iy-C.Iz)*ph_dot*th_dot;
        %% Gravity Matrix
        Gb=zeros(8,1);
        Gb(3)=-C.m0*C.g;
        %% Force function
